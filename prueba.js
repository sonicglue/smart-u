angular.module('systemax')
   ...
   * @description
   */
  .controller('GuaranteeListRAsigment', [
    '$rootScope',
    '$scope',
    '$state',
    'Guarantee',
...
/******************************************************************************/
   ...
   * @description
   */
  .controller('GuaranteeFormRAsigment', [
    '$scope',
    '$state',
    'Billing',
    'Guarantee',
    ...
    function($scope, $state, Billing, Guarantee, Stock, Product) {
      ...
      function updateStock(idstock) {
        console.log('lo que voy a actualizar', $scope.isRefurb);
        var isRefurb = $scope.isRefurb;
        console.log('mi idstock', idstock);

        // $scope.guarantee.status = 'check-in';
        Stock.prototype$patchAttributes({id: idstock},{isRefurb:isRefurb} )
          .$promise
          .then(
            function(guarantee) {
              console.log('esto se actualizo',guarantee);
        ...
        Guarantee.findById({id: guaranteeId, filter: guaranteeFilter})
          ...
          .then(
            function(guarantee) {
              ...
              // Obtener datos del vendedor
              if (guarantee.stock.user.type === 'seller') {
                $scope.guarantee.stock.seller = {
                  name: guarantee.stock.user.name + ' ' + guarantee.stock.user.lastName};
              }
            },
            function(err) {
              console.log(err);
