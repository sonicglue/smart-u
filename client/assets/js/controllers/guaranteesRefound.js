  ...
  $scope.$watch('formFilters', function(imei) {
    ...
    console.log('managernbvhbd',$scope.newIm);
      ...
      function updateStock(idstock) {
        ...
        // $scope.guarantee.status = 'check-in';
        Stock.prototype$patchAttributes({id: idstock},{   status:'sold', confirmSale:'true', args:{ data: { description: "Garantía entregada al cliente", status:'sold' }}
         } )
          .$promise
          .then(
            function(guarantee) {
      ...
      $scope.guaranteeStatus = function(status) {
        switch (status) {
          ...
          case 'refound': return 'Devolución';
          default: return '';
        }
      };// End guaranteeStatus
      /**
