angular.module('systemax')
  ...
  */
  .controller('GuaranteeListEntry', ['$rootScope', '$scope', '$state', 'Guarantee', 'Stock', 'Warehouse', function($rootScope, $scope, $state, Guarantee, Stock, Warehouse) {
  ...
  */
  .controller('GuaranteeFormEntry', ['$scope','$rootScope', '$state', 'Billing', 'Guarantee', 'Stock', 'Product', 'Failure', function($scope,$rootScope,  $state, Billing, Guarantee, Stock, Product, Failure) {
      // Variables generales
      var redirect = false;
        ...
        Guarantee.findById({id: guaranteeId, filter: guaranteeFilter})
          ...
          .then(
            function(guarantee) {
              ...
              console.log('guarantee', $scope.guarantees);
      ...
      function processGuarantee(guarantee) {
        ...
        if (guarantee.logs) {
          ...
          if (checkin) {
            ...
            console.log('dato que no sale', $scope.stock.dateCheckIn);
          }
        }
        // Activar casillas de los accesorios
        var tmp = '';
        _.each(guarantee.accessories, function(a) {
          console.log('accesorios', a);
        ...
        console.log('accesorios obtenidos', $scope.fallas);
  ...
  $scope.$watch('formFilters', function(imei) {
    ...
    /**
      ...
      function updateGuaranteee(guaranteeId) {
        console.log('lo que voy a actualizarppppppp', guaranteeId);
     ...
     function updateStock(idstock) {
        ...
        console.log('lo que voy a actualizarooooo',isRefurb, clas, newI  );
      ...
      function validateStock(stock, flag) {
        console.log('lo que tengo en validar estok', stock);
        if(stock != null){
        var today = $scope.getDateFormat(new Date(), 'x')
      ...
      $scope.addFailure = function(failure) {
        console.log('entro a addfailure con', $scope.guarantee.failures);
        var find = _.find($scope.guarantee.failures, function(f) {
          return f.idFailure === failure.id;
        });
        console.log('dentro de agregar fallas esta', find );
        if (!find) {
