angular.module('systemax')
  ...
  */
  .controller('GuaranteeListCanceled', ['$rootScope', '$scope', '$state', 'Guarantee', 'Stock', 'Warehouse', function($rootScope, $scope, $state, Guarantee, Stock, Warehouse) {
      var ctx = document.getElementById('myChart').getContext('2d');
      var days = ['domingo', 'lunes', 'martes', 'miércoles', 'jueves',
        'viernes', 'sábado']
      var labels = ['Registro', 'Recepción', 'Ingreso', 'Con el proveedor',
        'Re ingreso', 'Re asignación', 'Devolución', 'Cancelada'];
      var colors = ['rgb(245, 85, 85, 0.75)', 'rgb(115, 115, 225, 0.75)',
        ...
        'rgb(245, 185, 85, 0.75)', 'rgb(102, 0, 0, 0.75)'];
      var barCharData = {
            labels: [],
            datasets: []
          };
      ...
      function getGuaranties() {
        if ($scope.currentUser) {
          ...
          .then(
            function(response) {
              $scope.guaranties = response;
              console.log('garantias',response)
            /*  _.each(response, function(events){
                console.log('recorriendo eventos',events);
                var array ={
                  ticket: events.guideNumber,
                  model: events.stock.product.name,
                  imei: events.stock.IMEI,
                  seller: events.stock.user.name,
                  client: events.stock.client.name,
                  create: events.logs[0].date,
                  checkIn: events.logs[1].date,
                  entry: events.logs[2].date,
                  departure: events.logs[3].date,
                  reEntry: events.logs[4].date  ,
                  reAssignment: events.logs[5].date,
                 // refund: events.logs[6].date
                }
                $scope.event.push(array);
              })*/
        ...
        Guarantee.tracing({warehouseId: $scope.currentUser.idWarehouse,
          ...
          .then(
            function(data) {
              ...
              else if ($scope.class === 'guarantee-canceled') {
                barCharData.datasets.splice(0, 4);
              }
              chart.update();
      ...
      $scope.getTitle = function() {
        switch ($scope.class) {
          ...
          case 'guarantee-canceled': return 'Cancelada';
        }
      };// End getTitle
  ...
  */
  .controller('GuaranteeFormCanceled', ['$scope', '$state', 'Billing', 'Guarantee', 'Stock', function($scope, $state, Billing, Guarantee, Stock) {
      // Variables generales
      var redirect = false;
        ...
        Guarantee.findById({id: guaranteeId, filter: guaranteeFilter})
          ...
          .then(
            function(guarantee) {
              ...
              /*
              if (guarantee.status === 'open') {
                // ...si es el ingreso a pre-almacén
                processGuarantee(guarantee);
              } else {
                $scope.guarantee = guarantee;
                validateStock(guarantee.stock);
  ...
  $scope.$watch('formFilters', function(imei) {
    ...
    console.log('managernbvhbd',$scope.newIm);
     ...
     /* function updateStock(idstock) {
        ...
        console.log('lo que voy a actualizar',isRefurb, clas, newI  );
        // $scope.guarantee.status = 'check-in';
        Stock.prototype$patchAttributes({id: idstock},{isRefurb:isRefurb, class:clas, IMEI:newI, status:'active', reguardedNote:'Equipo Nuevo Reacondicionado'} )
          .$promise
          .then(
            function(guarantee) {
              console.log('esto se actualizo',guarantee);
